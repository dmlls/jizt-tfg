...
\vspace{-0.5cm}
En el anterior ejemplo, si decodificamos los \emph{tókenes} correspondientes a la palabra \texttt{``brutality''}, esto es, \texttt{[14506, 485]}, obtenemos los fragmentos \texttt{``brutal''} e \texttt{``ity''}, respectivamente. Análogamente, la palabra \texttt{``backbone''}, se descompone en \texttt{``back''} y \texttt{``bone''}.
