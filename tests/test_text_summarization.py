...
def test_add_prefix_to_subdivs():
    ...
    for pref in prefixes:
        assert [pref + sent for sent in sentences] == \
            ts.SplitterTokenizer._add_prefix_to_subdivs(sentences, pref)

def test_balance_subdivisions():
    subdivs = [["word"]*5]*9
    # Nothing to balance
    assert subdivs == ts.SplitterTokenizer._balance_subdivisions(subdivs, 5)
    #####################
    subdivs = [["word"]*5]
    ...
    assert [["word"]*5]*9 == ts.SplitterTokenizer._balance_subdivisions(subdivs, 5)
    #####################
    ...
    assert balanced_subdivs == ts.SplitterTokenizer._balance_subdivisions(subdivs, 10)
    #####################
    # check that the sentences passed to the function remain intact
    subdivs_before = copy.deepcopy(divided_sents)
    balanced_subdivs = ts.SplitterTokenizer._balance_subdivisions(divided_sents, 25)
    assert subdivs_before == divided_sents 

def test_divide_eagerly():
    assert [["word"]*5]*9 == ts.SplitterTokenizer._divide_eagerly(["word"]*5*9, 5)
    #####################
    print(divided_sents)
    max_len_subdiv = 25
    assert divided_sents == ts.SplitterTokenizer._divide_eagerly(sentences, max_len_subdiv)
...
def test_encode(initialize_bart, initialize_t5):
    ...
    divided = ts.SplitterTokenizer._divide_eagerly(preprocessed_text, max_len)
    balanced = ts.SplitterTokenizer._balance_subdivisions(divided, max_len)
...
def test_init(initialize_bart, initialize_t5):
    ...
    with pytest.raises(NotImplementedError):
        ts.SplitterTokenizer(tp.TextPreprocessor())